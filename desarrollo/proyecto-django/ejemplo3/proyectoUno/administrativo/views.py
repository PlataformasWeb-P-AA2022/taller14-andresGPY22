...
def index(request):
    ...
    # en el template
    informacion_template = {'propietarios': propietarios, 'numero_propietarios': len(propietarios)}
...
def obtener_propietario(request, id):
    ...
    # se agregará la información que estará disponible
    # en el template
    informacion_template = {'propietario': propietario}
...
def crear_departamento_propietario(request, id):
    """
    """
    propietario = Propietario.objects.get(pk=id)
    if request.method=='POST':
        formulario = DepartamentoPropietarioForm(propietario, request.POST)
        print(formulario.errors)
        if formulario.is_valid():
            formulario.save()
            return redirect(index)
    else:
        formulario = DepartamentoPropietarioForm(propietario)
